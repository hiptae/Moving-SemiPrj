...
<script>
	$(function() {
		...
		$("#deleteMsg").click(function() {
			...
			var receiveMsgnos = [];
			var sendMsgnos = [];
		if($("#v-pills-receive-tab").hasClass("active")) {
			$("li :checkbox:not(.sendLeader, .receiveLeader):checked").each(function() {
				receiveMsgnos.push($(this).val());
			})
		}	
		else {
			$("li :checkbox:not(.sendLeader, .receiveLeader):checked").each(function() {
				sendMsgnos.push($(this).val());
			})
		}
		console.log("receiveMsgnos : " + receiveMsgnos)
		console.log("sendMsgnos : " + sendMsgnos)
		
		if(receiveMsgnos.length > 0) {
			$.ajax({
			  method : "post",
			  url : '/message/remove',
			  traditional : true,
			  data : {receiveMsgnos : receiveMsgnos}
